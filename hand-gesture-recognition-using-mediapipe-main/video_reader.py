...
def main():
    ...
    for mode in range(1,2):
        for number in range(len(labels[mode])):
            print(f'{mode_list[mode]}{number}.avi')
            nb_frame = 0
            nb_frame_hand = 0
            cap = cv.VideoCapture(f'video_data/{mode_list[mode]}{number}.avi')
            while True:
                fps = cvFpsCalc.get()
                ...
                if results.multi_hand_landmarks is not None:
                    ...
                    if mode == 0: #Detection d'une main
                        nb_frame_hand += 1
                        for hand_landmarks, handedness in zip(reversed(results.multi_hand_landmarks),
                                                            reversed(results.multi_handedness)):
            ...
            print(f"Detection de main : {nb_frame_hand} / {nb_frame}")
...
def logging_csv(number, mode, landmark_list):
    ...
    if mode == 0 and (0 <= number <= 9):
        csv_path = 'model/keypoint_classifier/keypoint_video.csv'
...
def logging_csv_two_hands(number, mode, landmark_list):
    ...
    if mode == 1 and (0 <= number <= 9):
        csv_path = 'model/two_hands_keypoint_classifier/two_hands_keypoint_video.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])
    return
